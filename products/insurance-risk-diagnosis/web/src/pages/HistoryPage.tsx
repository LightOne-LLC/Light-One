  ...
  return (
    <div className="max-w-3xl mx-auto py-8 sm:py-12 px-4">
      <div className="flex items-center justify-between gap-3 flex-wrap mb-8">
        <div>
          <p className="text-[11px] font-medium tracking-[0.2em] uppercase text-gold mb-1.5">Your Financial Journey</p>
          <h1 className="text-2xl font-bold tracking-tight text-navy">診断履歴</h1>
        </div>
        <Link to="/diagnosis">
      ...
      <div className="space-y-3">
        {items?.map((item) => {
          ...
          return (
            <Card key={item.id} className="hover:border-navy/20 transition-colors">
              <Link to={`/result/${item.id}`} className="block">
                <div className="flex justify-between items-center gap-4">
                  <div>
                    <p className="text-xs text-ink-muted mb-1">{new Date(item.createdAt).toLocaleString('ja-JP')}</p>
                ...
                <p className="text-xs text-ink-muted mt-1">必要死亡保障額の目安: {formatManYen(item.requiredDeathCoverage)}</p>
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {item.topRisks.map((r) => (
                    <Badge key={r.label}>{r.label}</Badge>
                  ))}
                </div>
                <p className="mt-3 text-xs text-navy font-medium">結果を見る →</p>
              </Link>
              <div className="mt-2 flex justify-end">
                <button
                  type="button"
                  onClick={() => handleDelete(item.id)}
                  className="text-xs text-ink-muted hover:text-rose-600 transition-colors py-2 px-1 min-h-[36px]"
                >
                  削除
                </button>
              </div>
